...
| Worker 执行边界 | `WorkerCoordinator` 提供 Worker 注册、lease、幂等键、取消、过期回收和 Runtime `EffectExecutor` 适配；adapters 提供带 Bearer 鉴权的 HTTP claim/renew/complete/fail 与 polling Worker | `tests/worker-coordinator.test.ts`、`tests/worker-http.test.ts` | `d221466`、`2250df2`、`685be10` |
...
统一验证命令为 `pnpm exec tsc -b --pretty false && pnpm test`；当前结果为 44 个测试文件、189/189 通过，`pnpm build` 也已通过。
...
- 当前确定性门禁：`pnpm exec tsc -b --pretty false && pnpm test`，44 个测试文件、189 个测试通过；`pnpm build` 通过。Live Smoke 已执行到真实 HTTP 鉴权层并收到 `PROVIDER_HTTP_401`，未将其失败冒充内核证明。
...
| Worker 执行与迁移 | 本地 HTTP lease transport、Bearer 鉴权、polling Worker 和 Runtime 适配已实现 | 已验证真实 HTTP claim/renew/complete/fail、未授权拒绝、短 lease heartbeat、多 Worker 语义和 Runtime Effect 闭环；生产级 TLS、密钥轮换、共享 durable lease store、跨主机故障注入、Worker 迁移和网络分区仍需验证 |
