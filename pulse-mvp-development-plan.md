...
| Cross-runtime Session Store warm start | 增加可注入 `RuntimeSessionStore`、文件/SQLite durable 实现与 revision CAS；目标 Runtime 可按 `sessionId/globalVersion/relevanceRefs` 读取源快照，迁移选定 ResultRef、可见性、Privacy/Provenance，并推进结果 ID 水位 | `tests/warm-start.test.ts` | `34067fd`、`257ab69` |
...
- 当前确定性门禁：`npm test`，68 个测试文件、385 个测试通过；`npm run build` 与 `git diff --check` 通过。此前一次 Live Smoke 到达真实 HTTP 鉴权层并收到 `PROVIDER_HTTP_401`；本轮按当前环境重新尝试时在 DNS 阶段收到 `ENOTFOUND api.openai.com`，因此仍未把真实 Provider 证明写成通过。
