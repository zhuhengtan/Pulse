...
统一验证命令为 `npm exec tsc -b --pretty false && npm test`；当前结果为 47 个测试文件、231/231 通过，`npm run build` 和 `git diff --check` 也已通过。HTTP Worker 测试需要允许本机回环端口监听。
...
- 当前确定性门禁：`npm exec tsc -b --pretty false && npm test`，47 个测试文件、231 个测试通过；`npm run build` 通过。Live Smoke 已执行到真实 HTTP 鉴权层并收到 `PROVIDER_HTTP_401`，未将其失败冒充内核证明。
