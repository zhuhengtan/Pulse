...
统一验证命令为 `npm exec tsc -b --pretty false && npm test`；当前结果为 47 个测试文件、238/238 通过，`npm run build` 和 `git diff --check` 也已通过。HTTP Worker 测试需要允许本机回环端口监听。
...
- 当前确定性门禁：`npm exec tsc -b --pretty false && npm test`，47 个测试文件、238 个测试通过；`npm run build` 通过。Live Smoke 已执行到真实 HTTP 鉴权层并收到 `PROVIDER_HTTP_401`，未将其失败冒充内核证明。
...
| Worker 执行与迁移 | 本地 HTTP/HTTPS lease transport、Bearer 鉴权、polling Worker、请求超时、snapshot/restore、Runtime 适配和无重启 token 轮换已实现 | 已验证真实 HTTP/HTTPS claim/renew/complete/fail、未授权拒绝、短 lease heartbeat、token 重叠轮换、请求超时、in-flight lease 恢复、多 Worker 语义和 Runtime Effect 闭环；共享 durable lease store、跨主机故障注入和 Worker 迁移仍需验证 |
