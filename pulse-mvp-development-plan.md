...
| FactInbox 去重索引生命周期 | 当前保守保持全部已接收 `eventId`，不执行未经证明的 `seen` 裁剪 | 架构要求 exactly-once；现有 EventArchive 只有 seq 查询，尚无原子 dedupe ledger/watermark，因此长期运行的索引增长、checkpoint 后安全裁剪和跨进程 dedupe 仍需独立设计与验证 |
