...
| 输出预算与可恢复 Tool | `maxOutputTokens` 参与窗口预留、候选准入和 Provider 请求；structured schema 与最终 `outputSchema` 契约校验；保存 executionRef 并提供 RecoverableTool 对账入口 | `tests/provider-host.test.ts`、`tests/tool-host.test.ts`、`tests/m3-context-adapters.test.ts` | `d451014`、`b4461a8` |

统一验证命令为 `pnpm exec tsc -b --pretty false && pnpm test`；当前结果为 39 个测试文件、155/155 通过，`pnpm build` 也已通过。
...
- 当前确定性门禁：`pnpm exec tsc -b --pretty false && pnpm test`，39 个测试文件、155 个测试通过；`pnpm build` 通过。Live Smoke 已执行到真实 HTTP 鉴权层并收到 `PROVIDER_HTTP_401`，未将其失败冒充内核证明。
