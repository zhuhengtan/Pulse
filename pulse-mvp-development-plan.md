...
| Agent create policy / limits | `createAgent` 支持优先级、策略/限制引用与 `maxActiveLanes`；Agent 超时按注入 RuntimeClock 触发 `TIMEOUT`，配置和引用随 Agent 记录持久化；Fork admission 现在按 Agent 活动 Lane 数原子拒绝超限批次 | `tests/agent-create-contract.test.ts`、`tests/agent-creation.test.ts`、`tests/fork-affinity.test.ts` | `b2de59b`、`84bcb42` |
...
| Scheduler 软时间片与写者偏好 | 默认 `maxTickMs=5`；事实 Inbox、到期 Timer、queued Effect dispatch 和 Lane Step 共享软时间片，当前同步 Step 不被打断；`writerPreferenceBound` 限制排队写者前方 shared 插队数量；Timer 与 Executor completion 均只在 Runtime Tick 内结算；Runtime 派发的资源锁等待也进入公平队列，批次间用 `setImmediate` 让出事件循环 | `tests/m2-scheduler.test.ts`、`tests/runtime-control.test.ts`、`tests/resource-locks.test.ts`、`tests/storage-outbox.test.ts` | `e0eff57`、`62e0816`、`cfa9db2`、`6da19c5`、`6e0a748`、`ae85878`、`4a93ef7` |
| Runtime async single-writer boundary | Executor completion、LLM preparation 和异步 reconcile resolver 统一编码为 Fact，经过 FactInbox 与 Tick 才能修改 Effect/Result/Lane；`waitForIdle()` 覆盖事实排空与最终 durable save | `tests/provider-host.test.ts`、`tests/tool-host.test.ts`、`tests/storage-outbox.test.ts` | `e898425`、`634d78d`、`ae85878` |
...
| Filesystem/Shell 错误重试语义 | Filesystem 的沙箱、取消、基线冲突、缺失基线和锁超时错误，以及 Shell 的参数校验和进程启动错误均保留稳定 `code`/`retryable`；确定性错误不会被 Runtime 重试，锁超时仍可重试 | `tests/m3-context-adapters.test.ts` | `8223b90` |
...
| DSL 结构化 LLM 自纠错边界 | 结构化输出 schema、requirements、execution/retry policy 贯穿初始与纠错 Effect；原始输入和拒绝输出引用保留；默认最多一轮纠错，连续失败转 `OUTPUT_SCHEMA_VIOLATION`；真实上游 RuntimeError 不再被误包装为 schema correction | `tests/dsl-structured-contract.test.ts`、`tests/m4-dsl-e2e.test.ts` | `366ad32`、`a57f2fa` |
...
| HistoryCompaction 失败边界 | `$compact:apply` 校验 summarize Effect 的实际 Outcome；摘要失败或 Wait 未满足时 fail-closed，不清除压缩标记后静默继续业务步骤；压缩摘要继承被压缩前缀和摘要 Result 的最严格 Privacy/Taint | `tests/history-pressure.test.ts`、`tests/context-compaction.test.ts` | `60c2624`、`9934328` |
...
| HTTP Worker 直接调用错误契约 | `HttpWorkerClient.submit()` 对取消、远程 failed/cancelled 和控制/网络错误保留稳定 `code`、`retryable`、`details`，与 Runtime Effect 适配器保持一致 | `tests/worker-http.test.ts` | `027c5f3` |
...
| Fact Inbox durable dedupe archive | File/SQLite backend 提供 archive identity、连续 watermark、digest 校验和幂等 append；checkpoint 在 durable archive 确认后 compact，queued Fact 保持不 compact；restore 缺 archive 或 digest 不匹配时 fail-closed | `tests/fact-inbox-runtime.test.ts`、`tests/fact-inbox-persistence.test.ts`、`tests/sqlite-persistence.test.ts` | `5c7621c`、`1ffb2ad` |
...
| TimerWheel 异常安全 | 到期 Timer 取出后，callback 成功或取消才完成消费；callback 抛错时按原顺序重新进入 TimerWheel，避免静默丢失 | `tests/m2-scheduler.test.ts` | `d9f43bb` |
| 取消级联 Tick 预算 | Wait/cancel/Agent 收尾共享 Tick 预算；预算耗尽后通过 `setImmediate` 安排后续 Tick，不让 idle 提前结束 | `tests/tick-cascade-budget.test.ts`、`tests/runtime-control.test.ts` | `4179ce6` |
| Idle/Shutdown 到期 Timer 边界 | `waitForIdle()` 与 `shutdown()` 会处理当前已到期 Timer，但不会无条件快进未来 Timer | `tests/idle-boundary.test.ts` | `1815494` |
...
统一验证命令为 `npx tsc -b --pretty false && npm test`；最近一次允许本机 loopback 的结果为 73 个测试文件、473/473 通过，`npm run build`、`node benchmarks/deterministic.mjs`（serial/batch/lanes/coalesced 各 20/20 成功）和 `git diff --check` 也已通过。最近一次运行还覆盖了取消原因、失败 Lane Outcome、未决 Effect 传播、Observation gap 重同步、observation 字节上限、Runtime 配置、RuntimeConfig 调度/观察/Watchdog/预算/兼容性 fail-closed、恢复构造配置 fail-closed、恢复快照调度限制/游标/时间校验、Warm Start 版本/Privacy/ResultRef 引用校验、Provider loopback HTTP、Registered Runtime Provider path、Program Registry/ProgramRef、Runtime Model Registry/task route、Registered Model Adapter execution、模型 fallback 的 `maxAttempts` 上限、Provider HTTP retryability、Provider usage 指标合同、Provider response/finish reason/tool/usage fail-closed、Provider 网络错误与非流式非法 JSON 分类、Tool summary 超限/不可序列化回退、Session `warmStart.sessionId`、跨 Runtime Session Store warm start、文件/SQLite Session Store durable 恢复与 revision CAS、Persistence Backend 自动绑定 Session Store、目标 Host Policy 的云端候选重算及 Runtime 配置 fail-closed、Manifest workspace/network 权限与动态 ToolSet fail-closed、权限路径/主机规范化、Tool Manifest malformed contract fail-closed、external side-effect policy 的远程未知/取消/恢复处理、逻辑 ToolCall ID 稳定化、Action Decoder 工具隐私/来源传播、隐私感知日志导出、File/HTTP 审计日志 sink、Runtime `auditLogSink`/`auditLogPrivacy` 配置出口、注册 Tool 的默认 Runtime Executor 闭环、注册 external Tool 的 executionRef/reconcile 对账与结果 schema fail-closed、Runtime 默认 Tool 的非 JSON Artifact 发布、executionRef 输入准入、动态 ToolSet 查询合同、动态资源锁返回值合同、通用 Runtime reconcile 的异常/状态/输出 fail-closed、StoragePolicy 快照 limits/record/hash/pin 合同、Quarantine 快照与 reconcile-required Effect 一致性、Outbox 快照 identity/claim 合同、Model Registry 候选与任务路由注册合同、当前时刻 due timer 处理、结构化模型能力准入与 schema contract、推理能力下限路由、显式 contextSize 容量准入、Provider fetch/SSE cancellation 契约、Watchdog 二级策略变更与推理能力提升、Runtime Tool Registry、Agent create policy/limits、Agent active-lane admission、异步 reconcile Fact 边界、shutdown Fact 排空、开发模式纯 Step 守卫、DSL 只读 Context、ResultMeta 元数据和 history 稳定 hash、ReAct/结构化 DSL 输入与 provenance 契约、Worker HTTP 直接调用与适配器统一错误契约、架构示例 Agent Handle 的 `id` 兼容回归、Scheduler 默认软时间片/分阶段预算/写者偏好，以及 Timer 异常安全、取消级联预算、idle/shutdown 到期 Timer 和 Filesystem/Shell 确定性错误的 retryability 回归。受限沙箱中 HTTP Worker/Provider 测试会因禁止本机回环监听而返回 `EPERM`。
...
- `8223b90`：Filesystem/Shell 适配器补齐确定性错误的 `code`/`retryable` 语义；沙箱、取消、基线冲突、参数校验和进程启动失败不会被重试策略误判为可重试，锁超时保留可重试属性。
...
- 阶段性门禁记录：`npm test` 曾通过 68 个测试文件、409 个测试；同一阶段还记录过一次 Live Smoke 到达真实 HTTP 鉴权层并收到 `PROVIDER_HTTP_401`，随后按当前环境重新尝试时在 DNS 阶段收到 `ENOTFOUND api.openai.com`，因此真实 Provider 仍未证明通过。
- `36ba7a2`：Tool SDK、Runtime Tool Registry 和标准 Tool Adapter 对超限结构化摘要统一回退为“保留主结果、丢弃 summary”，并补充两条执行链回归；该阶段确定性门禁为 68 个测试文件、402/402 通过。
...
- `30aa32e`：Runtime Tool Registry 与 Tool SDK 对动态资源锁解析结果做结构校验，非法 `resource/mode` 在进入调度器前以 `INVALID_TOOL_RESOURCE_LOCKS` fail-closed。
- `f83acda`：InMemory Model Registry 拒绝非法/重复候选，Model Router 拒绝重复或非字符串任务路由候选，避免无效能力进入排序与 fallback。
- `4ce54bc`：Provider Adapter 入口校验 usage 的 token、latency、cached/uncached 边界和 cost 合同，非法指标不会进入 telemetry 或自适应路由反馈。
- `03b3c7a`：Tool executionRef、normalize 与 reconcile 输出统一执行严格 JSON 合同校验，Date、循环引用和其它非 JSON 值在持久化/对账前 fail-closed。
- `9304e66`：通用 Runtime reconcile Host API 将 resolver 异常、非法状态和非 JSON 输出统一转换为结构化 `unknown`，并保持 quarantine/Effect 状态不被提前改变。
- `744e174`：SessionStoragePolicy 对 limits、记录驻留状态、hash/字节数、pin source 和重复 key 做恢复前合同校验；非法快照不会替换现有 live state。
- `12556f3`：Persistence 快照校验 Quarantine 条目的唯一性、时间/原因字段及其 `reconcile_required + unknown` Effect 状态，避免恢复出脱节的隔离状态。
- `fcde94e`：Runtime 构造前统一校验调度、观察、Watchdog、预算和策略配置；负数、NaN、零间隔、非法枚举及反向 history limit fail-closed，合法的零容量边界保持可用。
- `3736127`：补齐 Watchdog 阈值、Tool/Policy/Router 版本及 persistence digest 的兼容性配置校验，避免恢复/观察逻辑接收空版本或非法数值。
- `528e810`：Outbox 恢复前校验 `effectId:attemptId` 身份关联、有限时间戳及 claimed/claimCount 一致性，拒绝脱节的跨进程投递记录。
...
- `89d4cee`：DSL `StepContext.history` 增加稳定记录级 hash，同时只暴露规范要求的引用、选择规则和隐私元数据，不暴露归档 output 正文。
- `1c94ca6`：HTTP Worker Client 统一区分鉴权/业务 4xx、超时、网络错误和 5xx 的 retryability；不可重试错误直接保留，不再被包装成 `remote_unknown`。
- `5ec3640`：HTTP Worker 不再把原生 `ECONNREFUSED` 等带 `code` 的网络异常误当成已分类错误，统一归一化为可重试的 `WORKER_HTTP_NETWORK_ERROR`。
- `027c5f3`：HTTP Worker 直接 `submit()` 路径保留远程 failed/cancelled、取消和控制错误的 `code/retryable/details`，与 Runtime Effect 适配器保持一致。
...
- `a57f2fa`：DSL ReAct 保留 results/findings/artifacts/events 到后续工具轮次；结构化 DSL 区分 rejected output 与真实上游 RuntimeError；StepContext 不再暴露内部 raw Result reader；metadata-only 查询不自动制造 provenance，数组根 Draft 也进入统一变更代理。
- `634d78d`：异步 reconcile resolver 的终态结果编码为 `effect_reconcile` Fact，只有 Runtime Tick 才能完成 quarantine Effect；同步 `reconcileEffect()` 继续保留兼容入口。
- `9934328`：history compaction 摘要继承被压缩前缀和摘要 Result 的最严格 Privacy/Taint，补齐 sparse history 的隐私回归。
- `84bcb42`：Fork admission 在创建子 Lane 前按 Agent 活动 Lane 数校验 `maxActiveLanes`，超限批次原子拒绝。
- `ae85878`：wake drain 改用 `setImmediate` 批次让出；`waitForIdle()` 纳入 Fact Inbox 和最终 durable save，避免异步结算后的后台持久化悬挂。
- `4a93ef7`：Runtime resource lock 的阻塞 Effect 登记公平等待队列，writer preference 对真实 Effect 派发生效，后来的 shared 不再绕过排队 writer。
- `3f37b8e`：`shutdown()` 纳入 Fact Inbox、LLM preparation 和 queued Effect 的收尾，已排队 Host Fact 不再让 shutdown 提前返回。
- `c80fac0`：按新的 Fact/Tick single-writer 边界校准 Host cancel、注册 Tool reconcile 和执行槽位回归测试。
- `d9f43bb`：TimerWheel 对失败 callback 做 selected 状态和可重试回滚，取消与重复消费边界有回归测试。
- `4179ce6`：取消级联、Wait 刷新和 Agent 收尾共享 Tick budget；预算耗尽后继续通过 `setImmediate` 推进，idle 不提前结束。
- `1815494`：`waitForIdle()` / `shutdown()` 会排空当前已到期 Timer，并保留未来 Timer 的等待边界。
- 当前确定性门禁：`npm test`，70 个测试文件、460 个测试通过；`npx tsc -b --pretty false`、`npm run build` 与 `git diff --check` 通过。`node benchmarks/deterministic.mjs` 已在本轮最终工作区重新运行，serial/batch/lanes/coalesced 各 20/20 成功；最近一次全量测试已覆盖 Executor completion Fact、coalesced wake、bounded Effect dispatch、persistence dispatch gate、planner-selected workers、异步 reconcile Fact、Agent lane admission、Runtime lock fairness、Timer 异常回滚、取消级联预算和 idle/shutdown 到期 Timer。
...
| FactInbox 去重索引生命周期 | 已完成 | File/SQLite durable dedupe ledger 提供 archive identity、连续 watermark、digest、幂等 append 和 fail-closed restore；checkpoint 只在 archive durable 后 compact，queued Fact 保留本地 `seen`。跨主机共享 archive 和生产数据库运维仍需宿主验证 |
