...
- 当前确定性门禁：`npm test`，68 个测试文件、437 个测试通过；`npx tsc -b --pretty false`、`npm run build` 与 `git diff --check` 通过。
