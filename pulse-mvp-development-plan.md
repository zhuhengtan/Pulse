...
| Filesystem 基线保护写入 | `FilesystemTool` 提供 SHA-256 `hash()` 与带跨进程锁、基线冲突检测、临时文件 + rename 的 `writeIfUnchanged()`；旧 `write()` API 保持兼容 | `tests/m3-context-adapters.test.ts` | `1488b0f` |
...
统一验证命令为 `npx tsc -b --pretty false && npm test`；当前结果为 54 个测试文件、311/311 通过，`npm run build` 和 `git diff --check` 也已通过。HTTP Worker 测试需要允许本机回环端口监听。
...
- 当前确定性门禁：`npm test`，54 个测试文件、311 个测试通过；`npm run build` 与 `git diff --check` 通过。此前一次 Live Smoke 到达真实 HTTP 鉴权层并收到 `PROVIDER_HTTP_401`；本轮按当前环境重新尝试时在 DNS 阶段收到 `ENOTFOUND api.openai.com`，因此仍未把真实 Provider 证明写成通过。
...
| 运行观测 | Runtime 侧已有只读出口、实时 ObservationInbox 镜像、JSONL/HTTP exporter、聚合和告警规则 | `inspect/explain`、`telemetry()`、`session.stream()`、JSONL/HTTP exporter 和有界聚合器已覆盖 Tool progress、Provider chunk、route 排除原因、provider/model slot、Attempt usage/cost、峰值与阈值告警；外部生产指标系统接入仍需宿主配置 |
