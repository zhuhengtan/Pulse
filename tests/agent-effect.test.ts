...
describe('built-in Child Agent Effect host', () => {
  ...
  it('keeps a detached child running after its parent is cancelled and exposes background scope state', () => {
    ...
    const childSettlement = runtime.mutationLog.entries.find((entry) => entry.mutations.some((mutation) => mutation.op === 'setEffect' && mutation.effectId === 'effect-1' && mutation.record.outcome?.status === 'succeeded'))
