...
export class StepBuilder<TState = JsonValue> {
  ...
  addWaitStep(name: string, spec: { dependencies: Array<{ key: string; target: { kind: 'lane' | 'effect'; id: string }; condition: 'success' | 'settled' }>; mode?: 'all' | 'any' | 'quorum'; quorum?: number; next: NextStepTarget }): this { this.handlers.set(name, () => ({ actions: [{ type: 'wait', spec: { ...spec, mode: spec.mode ?? 'all', ...(spec.quorum === undefined ? {} : { quorum: spec.quorum }), onUnsatisfied: 'resume_with_error', reason: 'dependency' } }], next: spec.next })); return this }
