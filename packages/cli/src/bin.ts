...
async function setupConfig(force: boolean, explicitPath?: string): Promise<void> { const path = explicitPath ?? process.env.PULSE_CONFIG ?? join(homedir(), '.pulse', 'config.json'); try { await access(path); if (!force) throw new Error(`CONFIG_EXISTS:${path}`) } catch (error) { if ((error as NodeJS.ErrnoException).code !== 'ENOENT' && error instanceof Error && error.message.startsWith('CONFIG_EXISTS:')) throw error } await mkdir(join(path, '..'), { recursive: true }); await writeFile(path, `${JSON.stringify({ provider: { provider: 'mock', model: 'mock', apiKeyEnv: 'OPENAI_API_KEY' }, approvalMode: 'ask', allowNetwork: false }, null, 2)}\n`, { mode: 0o600 }); await chmod(path, 0o600); process.stdout.write(`Wrote ${path}\n`) }
